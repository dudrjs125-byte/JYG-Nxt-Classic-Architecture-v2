...
const callGeminiLambda = async (content, noteId) => {
  ...
  try {
    ...
    console.error("Gemini Lambda 호출 중 오류:", error.response?.status, error.response?.data || error.message);
...
const callNovaLambda = async (content, noteId) => {
  ...
  try {
    ...
    console.error("Nova Lambda 호출 중 오류:", error.response?.status, error.response?.data || error.message);
...
app.get("/", (req, res) => {
  res.json({
    message: "선택 도우미 AI 서버 실행 중",
    status: {
      database: dbConnection ? "연결됨" : "연결 안됨",
      gemini: process.env.GEMINI_LAMBDA_URL ? "사용 가능" : "설정 안됨",
      nova: process.env.BEDROCK_LAMBDA_URL ? "사용 가능" : "설정 안됨",
    },
  });
});
...
app.post("/notes", checkDbConnection, async (req, res) => {
  ...
  dbConnection.query(sql, [content], (err, result) => {
    if (err) {
      console.error("고민 저장 중 오류:", err);
      return res.status(500).json({ error: "고민 저장 실패" });
    }
...
app.delete("/notes", checkDbConnection, async (req, res) => {
  ...
  dbConnection.query(sql, (err, result) => {
    if (err) {
      console.error("전체 기록 삭제 중 오류:", err);
      return res.status(500).json({ error: "전체 기록 삭제 실패" });
    }
...
app.post("/gemini-notes", checkDbConnection, async (req, res) => {
  ...
  if (!content?.trim() || !noteId) {
    return res.status(400).json({ error: "고민 내용과 ID가 필요합니다" });
  }
  ...
  try {
    ...
    dbConnection.query(updateSql, [aiResponse, noteId], (err, result) => {
      ...
      res.json({ message: "Gemini 추천이 완료되었습니다" });
    });
  } catch (error) {
...
app.post("/nova-notes", checkDbConnection, async (req, res) => {
  ...
  try {
    ...
    dbConnection.query(updateSql, [aiResponse, noteId], (err, result) => {
      ...
      res.json({ message: "Nova 추천이 완료되었습니다" });
    });
  } catch (error) {
